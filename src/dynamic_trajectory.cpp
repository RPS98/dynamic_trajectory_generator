...
static double getCummulativeTime(
...
References DynamicTrajectory::getReferences(
  ...
  const bool & lineal_optimization)
{
  ...
  for (auto & waypoint : next_trajectory_waypoint_) {
    ...
    waypoint.setTime(
      getCummulativeTime(segments, waypoint.getIndex()) +
      new_parameters_.global_time_last_trajectory_generated);
    index++;
  }
...
void DynamicTrajectory::generateTrajectory(
  const DynamicWaypoint::DynamicWaypoint::Deque & waypoints,
  bool force)
{
...
void DynamicTrajectory::swapTrajectory()
{
  ...
  if (from_scratch_) {
    ...
    parameters_mutex_.unlock();
    timeFittingWithVehiclePosition(
      new_trajectory, evaluateModifiedTrajectory(
        traj_, arg1, arg2));
  }
...
Eigen::Vector3d DynamicTrajectory::evaluateModifiedTrajectory(
  const ThreadSafeTrajectory & traj,
...
void resetWaypointThroughDeque(
  DynamicWaypoint::Deque & waypoints, const std::string & name,
  const Eigen::Vector3d & position)
{
  if (name == "") {
    return;
